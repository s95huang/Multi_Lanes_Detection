...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        self.num_lane_type = lane_num_types      #  bg + other type
        self.img_w = img_w
        self.n_strips = S - 1
        self.n_offsets = S
        ...
        ###self.invalid_mask.shape  1000,64,11,1
        ...
        ####multlly lane
        self.cls_layer = nn.Linear(2 * self.anchor_feat_channels * self.fmap_h, self.num_lane_type)
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        batch_features = self.feature_extractor(x)    #backbone output
        batch_features = self.conv1(batch_features)    # 1X1 conv
        batch_anchor_features = self.cut_anchor_features(batch_features)  #match anchor
        ...
        attention_matrix = torch.zeros(attention.shape[1],attention.shape[1], device=x.device)
        di = torch.arange(attention.shape[1])
        dj = di
        attention_matrix[di, dj] = 1.0
        attention_matrix = attention_matrix.repeat(x.shape[0], 1, 1)
        
        #attention_matrix = torch.eye(attention.shape[1], device=x.device).repeat(x.shape[0], 1, 1)
        non_diag_inds = torch.nonzero(attention_matrix == 0., as_tuple=False)   #torch.Size([999000, 3])
        attention_matrix[:] = 0     #torch.Size([1, 1000, 1000])
        print(attention.shape)       # torch.Size([1, 1000, 999])
        print(attention.flatten().shape)   # torch.Size([999000])
        
        
        ####  无法解决   这行代码被翻译成onnx.transopse  导致错误   暂无解决方法
        attention_matrix[non_diag_inds[:, 0], non_diag_inds[:, 1], non_diag_inds[:, 2]] = attention.flatten()
        
        batch_anchor_features = batch_anchor_features.reshape(x.shape[0], len(self.anchors), -1)
       
        
        attention_features_a = batch_anchor_features.permute(0, 2, 1)
        attention_features_b = attention_matrix.permute(0, 2, 1)
        attention_features_c = torch.bmm(attention_features_a, attention_features_b)
        attention_features = attention_features_c.permute(0, 2, 1)
        ...
        # Undo joining
        cls_logits = cls_logits.reshape(x.shape[0], -1, cls_logits.shape[1])   # bs, num_line, nums_line_type
        reg = reg.reshape(x.shape[0], -1, reg.shape[1])

        # Add offsets to anchors

        ##### multly lane type
        reg_proposals = torch.zeros((*cls_logits.shape[:2], self.num_lane_type + 3 + self.n_offsets), device=x.device)
        ...
        ##### multly lane type
        reg_proposals[:, :, :self.num_lane_type] = cls_logits
        reg_proposals[:, :, (self.num_lane_type+2):] += reg
        ...
        #p_proposals = reg_proposals[0]
        #scores = softmax(p_proposals[:, :self.num_lane_type])[:, 1:].max(dim=1)[0]
    ...
    def nms(self, batch_proposals, batch_attention_matrix, nms_thres, nms_topk, conf_threshold):
        ...
        for proposals, attention_matrix in zip(batch_proposals, batch_attention_matrix):
            ...
            with torch.no_grad():
                ####### multly lane
                scores = softmax(proposals[:, :self.num_lane_type])[:, 1:].max(dim=1)[0]
    ...
    def loss(self, proposals_list, targets, cls_loss_weight=10):
        ...
        for (proposals, anchors, _, _), target in zip(proposals_list, targets):
            ...
            if len(target) == 0:
                ...
                cls_pred = proposals[:, :self.num_lane_type]
            ...
            ###  positives_mask  [True, False, .....]  对应 proposal lane
            ...
            ######multly lane
            cls_target[:num_positives] = target[target_positives_indices][:, :self.num_lane_type].argmax(dim=1)
            cls_pred = all_proposals[:, :self.num_lane_type]

            # Regression targets
            reg_pred = positives[:, (self.num_lane_type+2):]
            with torch.no_grad():
                target = target[target_positives_indices]
                positive_starts = (positives[:, self.num_lane_type] * self.n_strips).round().long()
                target_starts = (target[:, self.num_lane_type] * self.n_strips).round().long()
                target[:, self.num_lane_type+2] -= positive_starts - target_starts
                all_indices = torch.arange(num_positives, dtype=torch.long)
                ends = (positive_starts + target[:, self.num_lane_type+2] - 1).round().long()
                invalid_offsets_mask = torch.zeros((num_positives, 1 + self.n_offsets + 1), dtype=torch.int)  # length + S + pad
                ...
                reg_target = target[:, (self.num_lane_type+2):]
    ...
    def compute_anchor_cut_indices(self, n_fmaps, fmaps_w, fmaps_h):
        ...
        # indexing
        unclamped_xs = torch.flip((self.anchors_cut[:, (self.num_lane_type+3):] / self.stride).round().long(), dims=(1,))
        unclamped_xs = unclamped_xs.unsqueeze(2)
        unclamped_xs = torch.repeat_interleave(unclamped_xs, n_fmaps, dim=0).reshape(-1, 1)
        cut_xs = torch.clamp(unclamped_xs, 0, fmaps_w - 1)
        unclamped_xs = unclamped_xs.reshape(n_proposals, n_fmaps, fmaps_h, 1)
        invalid_mask = ((unclamped_xs < 0) | (unclamped_xs > fmaps_w)) + 0
        cut_ys = torch.arange(0, fmaps_h)
        cut_ys = cut_ys.repeat(n_fmaps * n_proposals)[:, None].reshape(n_proposals, n_fmaps, fmaps_h)
        cut_ys = cut_ys.reshape(-1, 1)
        cut_zs = torch.arange(n_fmaps).repeat_interleave(fmaps_h).repeat(n_proposals)[:, None]
    ...
    def generate_side_anchors(self, angles, nb_origins, x=None, y=None):
        ...
        ######multly lane 》》》》  3 scores, 1 start_y, start_x, 1 lenght, S coordinates, score[0] = negative prob, score[1] = positive prob
        anchors = torch.zeros((n_anchors, self.num_lane_type + 2 + 1 + self.n_offsets))
        anchors_cut = torch.zeros((n_anchors, self.num_lane_type + 2 + 1 + self.fmap_h))
    ...
    def generate_anchor(self, start, angle, cut=False):
        ...
        if cut:
            anchor_ys = self.anchor_cut_ys
            anchor = torch.zeros(self.num_lane_type + 2 + 1 + self.fmap_h)
        else:
            anchor_ys = self.anchor_ys
            anchor = torch.zeros(self.num_lane_type + 2 + 1 + self.n_offsets)
        angle = angle * math.pi / 180.  # degrees to radians
        start_x, start_y = start
        anchor[self.num_lane_type] = 1 - start_y
        anchor[self.num_lane_type+1] = start_x
        anchor[(self.num_lane_type+3):] = (start_x + (1 - anchor_ys - 1 + start_y) / math.tan(angle)) * self.img_w
    ...
    def proposals_to_pred(self, proposals):
        ...
        for lane in proposals:
            lane_xs = lane[(self.num_lane_type+3):] / self.img_w
            start = int(round(lane[self.num_lane_type].item() * self.n_strips))
            length = int(round(lane[self.num_lane_type+2].item()))
            end = start + length - 1
            ...
            lane = Lane(points=points.cpu().numpy(),
                        metadata={
                            'start_x': lane[self.num_lane_type+1],
                            'start_y': lane[self.num_lane_type],
                            'conf': lane[:self.num_lane_type].max(),
                            'type': lane[:self.num_lane_type].argmax()
                        })
            lanes.append(lane)
        return lanes
    ...
    def decode(self, proposals_list, as_lanes=False):
        ...
        for proposals, _, _, _ in proposals_list:
            proposals[:, :self.num_lane_type] = softmax(proposals[:, :self.num_lane_type])
            proposals[:, self.num_lane_type+2] = torch.round(proposals[:, self.num_lane_type+2])
            if proposals.shape[0] == 0:
                decoded.append([])
                continue
            if as_lanes:
                pred = self.proposals_to_pred3(proposals)
            else:
                pred = proposals
            decoded.append(pred)
